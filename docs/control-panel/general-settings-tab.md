...
**A real, disclosed trade-off — At System Startup specifically:** seat passwords are DPAPI-protected (see [User Account for Workstation](user-account-for-workstation.md)) with `DataProtectionScope.LocalMachine`, not `CurrentUser`. That switch is what lets this mode work at all: the At-System-Startup task runs as `SYSTEM`, a different Windows account from whoever configured the seat, and `CurrentUser`-scoped data is decryptable *only* by the exact account that encrypted it — `SYSTEM` never would have been able to read it back. `LocalMachine` scope is keyed to the machine instead of one account, so `SYSTEM` (and any other local account/process) can decrypt it. The cost: a seat's saved password is no longer a secret from other local accounts on this machine, only from anything outside it — see `SeatCredentialProtector`'s doc comment for the full reasoning. Passwords saved under the old `CurrentUser` scope, from before this switch, still decrypt transparently (`Unprotect` tries `LocalMachine` first, falls back to `CurrentUser`), so existing seat configurations don't break.
