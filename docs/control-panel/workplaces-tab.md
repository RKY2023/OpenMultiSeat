...
- **"Indicate device" is a different real mechanism per tile type**, not one uniform click:
  - **Keyboard/Mouse:** press any physical key or move any physical mouse — the window registers for Win32 Raw Input with `RIDEV_INPUTSINK` for as long as it's open (not focus-scoped; see below) and blinks whichever tile's device generated the event, correlated back through the same device-path→`StableId` lookup `HidDeviceEnumerator` uses at scan time.
